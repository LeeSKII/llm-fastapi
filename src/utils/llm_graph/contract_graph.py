...
def check_contract_belong(state: dict)->State:
    ...
    try:
        ...
        system_prompt = dedent(f"""你是一个精确的合同查询判断机器，请根据用户提供的合同信息和用户提出的问题，判断合同是否属于用户查询范围。
                               ...
                               2.用户提出的可能是覆盖多个合同和设备的需求，但是每次你只会分析一个合同是否属于用户的查询范围，因此如果只要提供的合同属于用户的查询范围，则输出true。
                               3.只有明确发现合同中的不属于用户的查询范围，例如项目名称明显不相关或者没有相关的设备查询或者其它匹配的查询信息，才会输出false。
                               4.如果用户的查询包含了项目和设备，优先以项目名称作为判断依据，因为用户希望查询的是某个合同中的设备采购信息，因此需要使用项目名称作为唯一约束，不需要设备信息。
                               5.只有用户明确的在查询中指出多个查询条件是或者、or的关系，否则都优先确定一个查询条件的优先级，例如某项目名称|某设备名称。
                               # 示例：
                               1.用户提问：湖南华菱涟钢余热锅炉的合同有哪些；提供的合同数据：山东泰山钢铁余热锅炉改造工程EPC总承包余热回收项目。
                               分析：虽然两项目都涉及了余热锅炉设备，但是用户查询的是湖南华菱涟钢项目上的余热锅炉设备，而非其它项目，因此判断不属于用户的查询范围，judgement为false。
                               # 输出格式为json
